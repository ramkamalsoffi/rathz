    ...
    <section className="section-padding bg-black border-t border-zinc-900">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-20 text-center">
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-zinc-500 mb-6 italic">Core Problems We Solve</h2>
            <h3 className="text-3xl md:text-5xl font-bold tracking-tight text-white mb-2 leading-tight">Designed for Exceptional <br /> <span className="text-zinc-500 italic">Business Clarity</span></h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0 md:gap-y-12">
          {problems.map((problem, index) => (
            <div 
              key={index} 
              className="group relative flex flex-col justify-center py-8 md:py-14 border-b border-zinc-900 overflow-hidden cursor-pointer w-full"
            >
              <div className="relative z-10 flex flex-col gap-2 pr-12">
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 group-hover:text-primary transition-colors duration-500">
                  {problem.subtitle}
                </span>
              ...
              <div className="absolute right-0 top-1/2 -translate-y-1/2 z-10">
                 <div className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center group-hover:bg-primary group-hover:border-primary group-hover:rotate-45 transition-all duration-500">
                    <ArrowUpRight className="w-5 h-5 text-zinc-500 group-hover:text-white transition-colors duration-500" />
                 </div>
              </div>
